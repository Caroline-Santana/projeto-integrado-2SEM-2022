...
const insertBebida = async function (bebida){
    ...
    try {  
        ...
        let sql = `insert into tbl_bebida (nome, valor, foto)
                           values ('${bebida.nome}', '${bebida.valor}', '${bebida.foto}');`
...
const selectAllBebidas = async function (){
   ...
   const rsBebidas = await prisma.$queryRaw `select tbl_bebida.id as idBebida, tbl_bebida.nome as nomeBebida, tbl_bebida.valor as valorbebida, tbl_bebida.foto as fotoBebida,
...
//SOLUCIONAR COMO VOCE VAI APAGAR A PIZZA NA TABELA DE ORIGEM PORQUE POR CAUSA DE ELA SER UMA FOREIGH KEY NAO DA PARA DAR UM DROP NORMAl
...
const selectBebidaById = async function (id){
   ...
   let sql = `select nome, round (valor,3) as valor, foto from tbl_bebida 
   where id= ${id};`
