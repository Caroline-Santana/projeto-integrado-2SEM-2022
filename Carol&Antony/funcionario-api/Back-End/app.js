...
/****************************************************************
*    Rotas para autenticar login e deletar na tabela intermediar*
*    Data: 10/12/2022                                           *
*****************************************************************/
app.post('/v1/funcionario/login', cors(), jsonParser, async (request, response, next) => {
    ...
    if (headerContentType == 'application/json') {
        ...
        if (JSON.stringify(dadosBody) != '{}') {
            ...
            const dadosFuncionario = await controllerFuncionario.autenticando(dadosBody)

            statusCode = dadosFuncionario.status
            message = dadosFuncionario.message
...
app.delete('/v1/pizzatamanho/:id', cors(), jsonParser, async function (request, response){
    let statusCode
    let message 
    let id = request.params.id
        if(id != '' && id != undefined){
            const controllerTamanhoPizza= require('./controller/controllerTamanhoPizza.js')
            const deletarTamanhoPizza = await controllerTamanhoPizza.deletarPizzaTamanho(id)
            statusCode = deletarTamanhoPizza.status
            message = deletarTamanhoPizza.message
        }else{
            statusCode = 400
...
app.get('/v1/promocoes', cors(), async function (request, response){
...
//EndPoint para listar todas as mensagens do cliente
app.get('/v1/mensagens', cors(), async function (request, response){
    ...
    const dadosMensagens = await controllerMensagens.listarMensagens()
